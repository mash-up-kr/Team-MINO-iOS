...
struct ProfileSetupReducerTests {
    ...
    @Test("L1 — 한글·영문이 아닌 문자(숫자·특수문자·공백 포함)가 섞이면 저장이 막히고 에러를 표시한다")
    func nameChanged_disallowedCharacters_blocksSaveAndShowsError() async {
        for invalid in ["민호1", "민호!", "민호😀", "みんほ", "Мино", "이 지훈", "mi no", "민 호", "민호 Mino", " 민호", "민호 "] {
            let store = makeCreateTestStore()
    ...
    @Test("L1 — 최대 길이(15자)까지는 저장이 활성이다 — 한글·영문 둘 다")
