...
public final class MockPinRepository: PinRepository {
    ...
    public func pins(rooms: [Room], filter: PinFilter) async throws -> [Pin] {
        rooms.flatMap { Self.makePins(for: $0, page: 0, filter: filter) }
    }

    public func pins(room: Room, page: Int, filter: PinFilter) async throws -> [Pin] {
        Self.makePins(for: room, page: page, filter: filter)
    }
    ...
    private static func makePins(for room: Room, page: Int, filter: PinFilter) -> [Pin] {
        let now = Date.now
