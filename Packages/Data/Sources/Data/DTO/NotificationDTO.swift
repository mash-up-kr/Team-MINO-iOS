...
///
/// 서버는 장소 대상 알림에 `placeId`(장소 마스터 id)와 `pinId` 를 **둘 다** 싣는다(백엔드 확인
/// 2026-09-01). 그중 **`pinId` 만 읽는다** — 이동은 핀 단위이고, 하나의 장소가 여러 방에 저장되면
/// 핀이 여러 개라 장소 id 만으로는 어디를 열지 정해지지 않는다. 어느 핀을 고를지는 서버가 유형별
/// 정책(최근 저장 / 코멘트 최다)으로 판단해 `pinId` 로 준다.
///
/// `placeId` 는 선언하지 않는다 — 읽을 곳이 없고, `Decodable` 이 선언 안 된 키를 버리므로
/// 파싱에도 지장이 없다. 장소 단위로 뭔가 해야 할 기획이 생기면 그때 추가한다.
