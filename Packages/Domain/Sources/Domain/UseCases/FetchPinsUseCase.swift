...
public protocol FetchPinsUseCase: Sendable {
    /// 방 id 목록을 받아, 각 방의 핀을 목록 순서대로 이어붙인 평면 배열을 반환한다(초기 로드).
...
public struct DefaultFetchPinsUseCase: FetchPinsUseCase {
    ...
    public func execute(roomIDs: [RoomID]) async throws -> [Pin] {
        try await repository.pins(roomIDs: roomIDs)
    }

    public func execute(roomID: RoomID, page: Int) async throws -> [Pin] {
        try await repository.pins(roomID: roomID, page: page)
    }
}
