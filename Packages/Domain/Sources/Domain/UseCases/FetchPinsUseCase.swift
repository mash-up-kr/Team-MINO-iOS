...
public protocol FetchPinsUseCase: Sendable {
    /// 방 목록과 조회 기준(필터 칩)을 받아, 각 방의 핀을 방 순서대로 이어붙인 평면 배열을 반환한다.
...
public struct DefaultFetchPinsUseCase: FetchPinsUseCase {
    ...
    public func execute(rooms: [Room], filter: PinFilter) async throws -> [Pin] {
        try await repository.pins(rooms: rooms, filter: filter)
    }

    public func execute(room: Room, page: Int, filter: PinFilter) async throws -> [Pin] {
        try await repository.pins(room: room, page: page, filter: filter)
    }
}
