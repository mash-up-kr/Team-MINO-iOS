...
public protocol PinRepository: Sendable {
    /// 방 목록과 조회 기준을 받아, 각 방의 핀을 방 순서대로 이어붙인 평면 배열을 반환한다(홈 카드 덱).
    func pins(rooms: [Room], filter: PinFilter) async throws -> [Pin]
    /// 특정 방의 다음 페이지 핀을 반환한다("이 방 장소 더 보기"). page 는 0 부터 시작하는 페이지 커서.
    func pins(room: Room, page: Int, filter: PinFilter) async throws -> [Pin]
}
