...
/// 공유 후보 조회만 재생하는 스텁. 저장은 다른 프로토콜(``SavePinRepository``)이라 여기 없다.
private struct StubShareTargetRepository: ShareTargetRepository {
...
struct FetchSavedRoomsUseCaseTests {
    ...
    func propagatesRepositoryError() async {
        let sut = DefaultFetchSavedRoomsUseCase(
            repository: StubShareTargetRepository(targets: [], error: .unknown)
        )
