...
public enum HomeAction: Equatable {
    ...
    case pinsLoaded(pins: [Pin], startRoomID: RoomID?)
...
private func switchFilter(
    ...
    return .run { send in
        do {
            let pins = try await fetchPins.execute(roomIDs: rooms.map(\.id), filter: filter)
...
private func persistIfRoomChanged(
    from previous: RoomID?,
    to state: HomeState,
    using useCase: LastViewedRoomUseCase
) -> Effect<HomeAction, HomeNav> {
    guard let roomID = state.currentRoom?.id, roomID != previous else { return .none }
    // LastViewedRoom 계약은 원시 String 이다(저장 매체 경계) — 여기서 풀어 넘긴다.
    return .run { _ in await useCase.save(roomID: roomID.value) }
}
...
public func homeReducer(
    ...
    { state, action in
        ...
        case .loaded(let rooms):
            ...
            return .run { send in
                do {
                    ...
                    async let pins = fetchPins.execute(roomIDs: ordered.map(\.id), filter: filter)
                    async let startRoomID = lastViewedRoom.load()
                    send(.pinsLoaded(pins: try await pins, startRoomID: (await startRoomID).map(RoomID.init)))
                } catch let error as DomainError {
                    send(.loadFailed(error))
                } catch {
                    send(.loadFailed(.unknown))
        ...
        case .tapMorePlaces:
            ...
            return .run { send in
                do {
                    let pins = try await fetchPins.execute(roomID: room.id, page: page, filter: filter)
