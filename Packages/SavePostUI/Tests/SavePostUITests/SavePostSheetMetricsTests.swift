...
struct SavePostSheetMetricsTests {
    ...
    func height_onDesignDevice(roomCount: Int, expected: CGFloat) {
        #expect(SavePostSheetMetrics.height(roomCount: roomCount, safeAreaBottom: 34) == expected)
    }

    /// 홈 인디케이터가 없는 기기(SE3)에서는 그만큼 낮아진다 — 화면 667 중 610.
    @Test("하단 safe-area 가 0 이면 34 만큼 낮아진다")
    func height_withoutHomeIndicator() {
        #expect(SavePostSheetMetrics.height(roomCount: 5, safeAreaBottom: 0) == CGFloat(610))
        #expect(SavePostSheetMetrics.height(roomCount: 3, safeAreaBottom: 0) == CGFloat(402))
    }
}
