...
/// 기본 placeholder 를 보여준다(현재 person=사람 아이콘, company·academy 는 전용 일러스트가 없어 빈 배경).
...
public struct MHAvatar<Badge: View>: View {
    ...
    private var cornerRadius: CGFloat { MHAvatar.cornerRadius(variant, size: size) }

    // 모서리: person=원(크기/2), company·academy=크기×0.25(홀수면 짝수로 올림, Figma 규칙).
    static func cornerRadius(_ variant: MHAvatarVariant, size: CGFloat) -> CGFloat {
        switch variant {
    ...
    // 이미지 없을 때 기본 표시. person=사람 아이콘(회색), 그 외는 빈 배경(전용 일러스트 미보유).
