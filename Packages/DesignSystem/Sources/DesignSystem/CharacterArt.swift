...
///
/// 아바타 슬롯(``MHAvatar``·``MHAvatarGroup``·``MHAvatarStack``·``MHComment``)·프로필 선택 그리드·
/// 마이페이지 큰 프로필이 모두 이 아트를 그린다. ``MHCharacter``(`character01`~`character12`) 는
/// 이전 세대 아트로 지금 그리는 화면이 없고, "선언 순서 = 저장되는 색" 계약의 기준으로만 남아 있다.
