...
| 그림자 | `.mhShadow(...)` |
| 캐릭터 아트 | `MHHomeMascot`, `MHRoomCover`, `MHAvatarProfile`, `MHCharacter` — `Image(_:)` / 지도 마커는 `Image(mapPin:state:)` |
| 컴포넌트 | `MHButton`, `MHChip`, `MHActionArea`, `MHTextField`, `MHTextArea`, `MHTextButton`, `MHSpinner`, `MHCharacterCounter`, `MHContentBadge`, `MHThumbnail`, `MHAvatar`, `MHAvatarGroup`, `MHAvatarStack`, `MHCategory`, `MHSnackbar`, `MHTooltip`, `MHMenu`, `MHRoomHeader`, `MHFilterBar`, `MHComment`, `MHHomeCard`, `MHCheckbox`, `MHRoomThumbnail`, `MHRoomCard`, `MHLocationCard`, `MHInvitationCard`, `MHInvitation`, `MHTabBar`, `MHTopNavigation`, `MHSelectionGrid`, `MHNotificationCell`, `MHStatusMessage`, `MHIllustratedMessage`, `MHPaginationDots` |
