...
struct InviteFriendsReducerTests {
    ...
    // 세션이 끊긴 실패에 "잠시 후 다시 시도" 를 권하면 거짓말이 된다 — 다시 눌러도 계속 401 이다.
    @Test("L2 — 세션 만료는 일반 실패와 다른 안내로 갈린다")
    func sessionExpiry_showsDistinctNotice() async {
        let store = makeStore(result: .failure(.unauthorized))

        await store.send(.tapCopyLink) { $0.isPreparingLink = true }
        await store.receive(.linkFailed(.unauthorized)) {
            $0.isPreparingLink = false
            $0.error = .unauthorized
        }

        #expect(store.currentState.notice == .sessionExpired)
        store.finish()
    }
    ...
    func dismissNotice_clearsBoth() async {
        ...
        await store.send(.linkFailed(.inviteCodeFetchFailed)) { $0.error = .inviteCodeFetchFailed }
