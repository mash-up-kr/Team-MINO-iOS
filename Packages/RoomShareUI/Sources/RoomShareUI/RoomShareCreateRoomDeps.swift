...
///
/// 이 시트를 띄우는 Feature 의 deps(``ArchiveDeps``·``HomeDeps``)가 이 프로토콜을 확장(refine)한다 —
/// 자식은 `createRoom` 하나만 보고(방 목록·핀·공유 저장 UseCase 는 안 보인다), 조립부(App)는
/// 지금처럼 Feature deps 하나만 준수하면 된다.
///
