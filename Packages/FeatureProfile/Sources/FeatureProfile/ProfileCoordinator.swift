...
public enum ProfileRoute: Hashable {
    /// 방 편집 (RoomCreationUI.RoomFormView) — 편집할 방을 **라우트가 싣는다**.
    ///
    /// Coordinator 프로퍼티로 넘기면 pop 뒤에도 값이 남아 다음 진입이 지난 방으로 열리고,
    /// `push` 와 store 팩토리 호출 순서에 조용히 의존하게 된다. 값이 네비게이션 엔트리와
    /// 같이 살고 같이 사라지도록 여기 담는다.
    case roomEdit(RoomDetailRoom)
}
...
public final class ProfileCoordinator: Coordinator {
    ...
    func makeRoomFormStore(room: RoomDetailRoom) -> RoomFormStore {
        let store = RoomFormStore(
            RoomFormState(
                mode: .edit,
                roomName: room.title,
                roomDescription: room.memo,
                selectedColorIndex: RoomColorPalette.index(ofHex: room.color)
            ),
