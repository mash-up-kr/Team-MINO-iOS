...
public struct DeeplinkBuilder: Sendable {
    ...
    public func webURL(for deeplink: Deeplink) -> URL? {
        makeURL(scheme: "https", host: configuration.host, segments: deeplink.segments)
    }
    ...
    public func appURL(for deeplink: Deeplink) -> URL? {
        ...
        guard let host = deeplink.segments.first else { return nil }
        return makeURL(scheme: configuration.scheme, host: host, segments: Array(deeplink.segments.dropFirst()))
    }

    private func makeURL(scheme: String, host: String, segments: [String]) -> URL? {
        guard segments.allSatisfy(Deeplink.isValidCode) else { return nil }
