...
extension Deeplink {
    ...
    /// 초대 코드의 **형식**(서버 스펙: 대문자·숫자 6자)은 여기서 강제하지 않는다. 합류 API 가 코드를
    /// 검증하므로 클라이언트가 같은 규칙을 복제하면, 서버가 형식을 넓힐 때 **앱만 멀쩡한 링크를
    /// 거절하게** 된다. 대신 형식과 무관하게 참인 것만 막는다 — 세그먼트를 쪼개는 문자(`/`·공백)와,
    /// 눈에 보이지 않아 사람이 링크를 눈으로 검증할 수 없게 만드는 제어·zero-width 문자
    /// (`AB<U+200B>12` 는 화면에서 `AB12` 와 구별되지 않는다).
