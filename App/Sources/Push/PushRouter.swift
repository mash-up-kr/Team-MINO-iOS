...
final class PushRouter: NSObject, UNUserNotificationCenterDelegate {
    let route = PendingHandoff<NotificationDestination>()
    ...
    nonisolated func userNotificationCenter(
        ...
        // Task 안으로 들고 들어갈 수 없다 — 격리 경계를 넘는 건 값이 정해진 목적지여야 한다.
        let destination = PushNotificationPayload.destination(
        ...
        Task { @MainActor in self.route.deliver(destination) }
    }
}
